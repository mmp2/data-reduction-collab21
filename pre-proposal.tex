...
The PI brings extensive experience in ML, from the statistical issues \cite{PerraultMMcQueen:nips17} to software \comment{\cite{mcqueenMVdpZ:megaman16,*mcqueenMVdpZ:megaman-jmlr16}} while the co-PIs are at the forefront of ML for spectroscopic data \cite{vanderplasConnolly:09}.

{\bf Previous work}

{\bf Existing support} No other funding exists for this project. \mmp{ The PI also has no current funding for general manifold learning. Closest related support consists of an IGERT funded student, and a collaborative grant of 170M CPU hours at the Argonne Labs Computing Facility (see CV) since September 2017 for the PI's research on manifold learning in chemistry.}
